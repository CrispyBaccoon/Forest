...
function Left() {
  ...
  this.drag_el.innerHTML = `\
      ...
      ${(() =>
        ["minimize", "maximize", "restore", "close"].map((control) => {
          return `<div id="${control}-window" class="button">
<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path id="${control}Icon" class="windowIcon"
        d="M8.00024 2C8.55102 2 9.08227 2.07227 9.59399 2.2168C10.1057 2.35742 10.5822 2.55859 11.0236 2.82031C11.469 3.08203 11.8732 3.39648 12.2365 3.76367C12.6037 4.12695 12.9182 4.53125 13.1799 4.97656C13.4416 5.41797 13.6428 5.89453 13.7834 6.40625C13.9279 6.91797 14.0002 7.44922 14.0002 8C14.0002 8.55078 13.9279 9.08203 13.7834 9.59375C13.6428 10.1055 13.4416 10.584 13.1799 11.0293C12.9182 11.4707 12.6037 11.875 12.2365 12.2422C11.8732 12.6055 11.469 12.918 11.0236 13.1797C10.5822 13.4414 10.1057 13.6445 9.59399 13.7891C9.08227 13.9297 8.55102 14 8.00024 14C7.44946 14 6.91821 13.9297 6.40649 13.7891C5.89477 13.6445 5.41626 13.4414 4.97094 13.1797C4.52954 12.918 4.12524 12.6055 3.75805 12.2422C3.39477 11.875 3.08227 11.4707 2.82055 11.0293C2.55883 10.584 2.35571 10.1055 2.21118 9.59375C2.07055 9.08203 2.00024 8.55078 2.00024 8C2.00024 7.44922 2.07055 6.91797 2.21118 6.40625C2.35571 5.89453 2.55883 5.41797 2.82055 4.97656C3.08227 4.53125 3.39477 4.12695 3.75805 3.76367C4.12524 3.39648 4.52954 3.08203 4.97094 2.82031C5.41626 2.55859 5.89477 2.35742 6.40649 2.2168C6.91821 2.07227 7.44946 2 8.00024 2Z"
        fill="#222222" />
</svg>
          </div>`;
        }))().join("")}\
  ...
  this.select_autocomplete = () => {
    if (
      this.selection.word.trim() !== "" &&
      this.suggestion &&
      this.suggestion.toLowerCase() !== this.active_word().toLowerCase()
    ) {
      this.autocomplete();
    } else {
      this.inject("\u00a0\u00a0");
  ...
  this.select_synonym = () => {
    if (this.synonyms) {
      this.replace_active_word_with(
        this.synonyms[this.selection.index % this.synonyms.length]
      );
      this.stats.update();
  ...
  this.autocomplete = () => {
    this.inject(
      this.suggestion.substr(
        this.selection.word.length,
