...
function Reader() {
  ...
  this.alert = function (t) {
    setTimeout((t) => {
      left.stats.el.innerHTML =
        "<b>Reader</b> Select some text before starting the reader.";
    }, 400);
  };
  ...
  this.run = function () {
    ...
    let html = ": ";
    html +=
      "<span style='opacity:0'>" + left.reader.orp_pad(words, orp) + "</span>";
    html +=
      "<span class='fm'>" +
      orp.before.trim() +
      "</span><span class='fh'>" +
    ...
    html +=
      ...
      parseInt(
        (left.reader.index /
          parseFloat(left.reader.queue.length + left.reader.index)) *
          100
      ) +
      "% " +
    ...
    left.reader.queue = left.reader.queue.splice(
      1,
      left.reader.queue.length - 1
    );
    left.reader.index += 1;
